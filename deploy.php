...
task('release:increaseVersionPatch', function () {
    ...
    file_put_contents('composer.json', json_encode($composerMetadata, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));

    $emconf = file_get_contents('ext_emconf.php');
    $emconf = preg_replace(
        '/[\'"]version[\'"]\s*=>\s*[\'"][0-9\.]*[\'"]/m',
        '\'version\' => \'' . $composerMetadata->version . '\'',
        $emconf
    );
    if (!empty($emconf)) {
        file_put_contents('ext_emconf.php', $emconf);
    }
});
...
task('release:commitComposer', function () {
    ...
    runLocally('git add ext_emconf.php');
